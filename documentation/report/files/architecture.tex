...
The OSNAP project consists of two libraries (ComLib and SocialLib) and various Android applications. This section describes
the Android software in detail. We have two types of Android software, Applications and Services. An Application is an active
program with a user interface while a Service is a background process that provides functionality for applications.

\subsection{OSNAP Generic Prototyping Application}
\begin{wrapfigure}{l}{20mm}
	\centering \includegraphics[scale=1]{img/app-generic}
\end{wrapfigure}
This application allows the user to scan a QR code and connect via Bluetooth to a remote device. It will then retrieve additional
metadata from the remote device and build a list of all the functionality the remote device supports. The user can then test each
function with the click of a button. There is also a download link to the OSNAP market on the internet that allows the user to
download a specialized application for the OSNAP product in question. The idea is that the user can use this application to
scan various OSNAP products and retrieve additional information or software for that specific product.

\subsection{OSNAP Jacket Application} \label{section:app-jacket}
\begin{wrapfigure}{l}{20mm}
	\centering \includegraphics[scale=1]{img/app-jacket}
\end{wrapfigure}
This application was developed to use and test the full functionality of both the ComLib and SocialLib in combination with the
Jacket Prototype. It connects to the Jacket via Bluetooth and uses any Social Network service installed and running on the
...
\subsection{Facebook Service} \label{section:service-fb}
\begin{wrapfigure}{l}{20mm}
	\centering \includegraphics[scale=1]{img/app-fb}
\end{wrapfigure}
This service will keep an active connection with Facebook with a specific user account that has been logged in previously. It will
broadcast status updates using intents according to the rules set up by for example the OSNAP Jacket Application (see section 
\ref{section:app-jacket}).
...
\subsection{Twitter Service}
\begin{wrapfigure}{l}{20mm}
	\centering \includegraphics[scale=1]{img/app-twitter}
\end{wrapfigure}
Similar to the Facebook Service (see section \ref{section:service-fb}), this background process monitors a Twitter feed for
any changes and broadcasts these to any listening applications.
