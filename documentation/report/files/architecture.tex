...
found in these networks such as Post, User, Comment and so on.biggest biggest 
...
\begin{figure}[h!]
\centering \includegraphics[width=1.0\textwidth]{img/sequenceDiagramSocialService.png}
\caption{Sequence Diagram Social Service}
...
\newpage
\section{Prototypes}
We have three different prototypes that use the oSNAP library. These three different prototypes serve both
as a proof of concept and to test that the library works. Each of the prototypes should be different
to show that the library can be used to quickly prototype Arduino Tangible User Interfaces.do we have a 
One of the protoypes, the T-Shirt, is significantly bigger and more complex than the other two.
The T-Shirt is the prototype for the entire project and the other two are
just proof that the library works for other applications.
...
statistics or updates. The shirt has many planned tangiable features (see figure \ref{fig:design-TShirt}):
...
Any of these output devices can be mapped to a fair deal of concepts found in social network.
...
This prototype will primarily be made to show communication going from the Arduino to the Android using our libraries.
...
This scenario is represented in figure \ref{fig:req-usecase1}
The following figure represents the communication between the applications.
...
the social application to the T-Shirt application, which is merely capable of receiving.
Also, both are implemented as Android applications.
...
specifying the behavior of the T-Shirt. This scenario is represent in figure \ref{fig:req-usecase2}
...
authenticate within the social network and setup the rules for the T-Shirt app.
...
It also implied that a mechanism to specify what content to fetch had to be provided by the Social library.
The following figure represents the communication between the services and the applications.
